...
| `Camera`, `Light`, `LightKind`, `ParticleBurst`, `Key`, `InputState` | **Frozen for 1.0** | View/light/particles + input; `MAX_LIGHTS` grew 4 → **256** in 3.0 (additive: same `Light` API, clustered on the GPU) |
| `Kerabit::environment`, `Context::set_environment` / `clear_environment`, `Entity::lod`, scene `environment` / `lods` | **3.0 additive** | HDR image-based lighting and per-entity LOD chains; omit for the procedural sky / single mesh |
